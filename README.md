...
<dl><dd>
 ...
 • SF = AS = Safari . A web-browser developed by Apple Inc. Uses WebKit engine. WebKit was closed-source & based-on OpenSource KDE-HTML(KHTML aka WebCore) layout engine & OpenSource KDE-JS(KJS aka JavaScriptCore) engine, etc. Later WebKit was made OpenSource by Apple Inc, and then Adobe Systems, KDE, Igalia, and others joined. Later JavaScriptCore was replaced with SquirrelFish JS (SFX) engine in WebKit. WebKit was internally upgraded into WebKit2. Many other web-browsers are actually based on modified/customized version of WebKit engine. WebKit devs have removed GC related codes, after Google forked WebKit & created (WebKit-based) Blink for GC. Apple Inc forcefully restricts other devs/groups, one of such is, they are forced to use WebKit to develop their apps for macOS/iOS, etc.<br/>
 ...
 • Ch = GC = Chrome = GCh = gCh = gCH = Google-Chrome . CM is developed by Google LLC . Proprietary & executes various binaries even before user logins into their OS, & does-not provide any option to completely disable/remove such malware-like behavior. GC web-browser portion is based-on Chromium web-browser. Initially used WebKit rendering engine, later Google eventually forked WebKit to create Blink engine, now all Google-Chrome variants are using Blink except in iOS, in iOS its forced to be based on WebKit.<br/>
 • OS = Operating System = OprtSys . A collection/set of vast numbers+types of programs, to assist interactions between computer User and computer hardwares & softwares and computer networks. Many OS did-not & does-not have GUI. Modern OS usually contains: microKernel/monolithicKernel/nanoKernel, HW layout mngr, HW drvr, IPC mngr, I/O mngr, Virt mem mngr, Protected mem mngr, Scheduling, PreEmptive MultiTasking, RealTime support, Console I/O, FS support, Net support, Sockets, Security monitoring, Process mngr, PnP mngr, Power mngr, Graphics Display mngr, Window mngr, system services, related System Libraries, Userland/UserApps, App Environments, Servers/Daemons/Services for Userland, OS SubSystems/Thread support, related Userland Libraries.<br/>
 • Unx = Unix = Research-Unix = &#42;nix. Originally developed (in 1969) by Ken Thompson & Dennis Ritchie & others at-then AT&T Bell Labs.<br/>
 • BSD = BSD-Unix = Berkeley Software Distribution . Initially BSD was based on Unix v5, & it was developed by Bill Joy & Ozalp Babaoglu & Keith Bostic & others from CSRG(Computer Systems Research Group). Uses Monolithic kernel. BSD was updated to 4.3BSD-Net2, and then ~finally to 4.4BSD-Lite2 (after winning the lawsuit filed-by AT&T). Then 2-devs released 386BSD based on 4.3BSD-Net2, ..., then that was forked-into NetBSD ,a pair of various devs/groups   a ~final release 4.4BSD-Lite2,386BSD, 4.4BSD-Lite2, FreeBSD,<br/>
 ...
 • Win = ❖ = Windows OS = WinOS . WinOS developed by Microsoft corp. ClosedSource GUI OS. Windows 9X & earlier WinOS used Monolithic kernel & was based on “Interface Manager”. WinCE-family & WinNT-family of WinOS used Hybrid(NT) Kernel. Initial WinOS was a Graphical-Shell of MS-DOS OS, initial MS-DOS was based on CP/M-OS, 86-DOS, etc. WinOS-10/Win10 includes MS-Edge web-browser, its initially based on Trident engine, then changed into Blink(CM) engine.<br/>
